...
const color = (text, color) => { return !color ? chalk.green(text) : chalk.keyword(color)(text) };
 ...
 //baner
const banner = cfonts.render("MINECRAFTBOT", {
  font: 'pallet',
  align: 'center',
  gradient: ["green","blue"]
})
...
async function startProo() {
  ...
  console.log(chalk.cyanBright("🔥 MINECRAFTBOT"));
...
⛏️ Prepara tu pico y tu espada, que las aventuras, los mobs y los cofres llenos de tesoros te esperan ⚔️💎

📜 No olvides leer nuestras reglas para no despertar a los creepers escondidos 💣👀
...
💌 「 ¡Enhorabuena! 🎉 Has ascendido a la mesa de los administradores 🪄, a trabajar rata」
...
const runtime = function(seconds) {
  ...
  registro: `
╔════◇◆◇════╗
💬 ❝ 𝑩𝒊𝒆𝒏𝒗𝒆𝒏𝒊𝒅𝒐 @${num.split('@')[0]} 🏰 al mundo de *${grup}* ❞
💬 ❝ Antes de explorar minas, derrotar mobs y abrir cofres llenos de tesoros 💎, debes registrarte ⛏️ ❞
💬 ❝ Escribe:  .reg  y prepárate para tu aventura ⚔️🪓 ❞
╚════◇◆◇════╝
`,
...
`,
  ...
  minecoins: `『 💰 ᴄᴏɪɴs ɪɴsᴜғɪᴄɪᴇɴᴛᴇs @${sender.split('@')[0]} 』`
}
...
💬 ❝ Hey @${num.split('@')[0]} 👋  
Antes de intentar cualquier cosa extraña, recuerda: busca tu **garbo** primero 🪄✨  
No intentes activar subbots ni usar \`.serbot\` 🚫👾, que aquí todos jugamos limpio ⛏️🛡️ ❞
`;

        // Aquí iría el envío del mensaje, por ejemplo:
        conn.sendMessage(m.chat, moneybot, MessageType.text);
    } catch (err) {
        console.error(err);
    }
    break;
...
try {
if(!q.trim()) return enviar(`Escriba su nombre para enviar con letras especiales, Ejemplo: !nick MINECRAFTBOT`);
        ...
        function Return(sul) {
             sat = JSON.stringify(sul, null, 2)
                  bang = util.format(sat)
                    ...
                    try {
    const expresion = budy.slice(3);
    const resultado = Function(`return (${expresion})`)();
    enviar(util.format(resultado));
} catch (e) {
    enviar(String(e));
}
